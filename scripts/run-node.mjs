...
const maybeBuildCnControlUi = async (deps) => {
  if (deps.env.OPENCLAWCN_DEV !== "1") {
    return true;
  }
  const controlUiRoot = deps.env.OPENCLAWCN_CONTROL_UI_ROOT?.trim();
  if (!controlUiRoot) {
    return true;
  }
  if (statMtime(path.join(controlUiRoot, "index.html"), deps.fs) != null) {
    return true;
  }

  logRunner("Building CN Control UI assets.", deps);
  const buildCommand =
    process.platform === "win32"
      ? { command: "cmd.exe", args: ["/d", "/s", "/c", "pnpm --dir ui-cn build"] }
      : { command: "pnpm", args: ["--dir", "ui-cn", "build"] };
  const build = deps.spawn(buildCommand.command, buildCommand.args, {
    cwd: deps.cwd,
    env: {
      ...deps.env,
      VITE_EDITION: "cn",
  ...
  return buildRes.exitCode === 0 || buildRes.exitCode === null;
};
...
export async function runNodeMain(params = {}) {
  ...
  try {
    ...
    if (!buildRequirement.shouldBuild) {
      ...
      if (!(await maybeBuildCnControlUi(deps))) {
        return await closeRunNodeOutputTee(deps, 1);
      }
