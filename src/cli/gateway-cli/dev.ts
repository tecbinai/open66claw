...
function resolveCnGatewayOverrides(env: NodeJS.ProcessEnv = process.env) {
  if (env.OPENCLAWCN_DEV !== "1") {
    return {};
  }
  const controlUiRoot = env.OPENCLAWCN_CONTROL_UI_ROOT;
  return {
    auth: {
      mode: "none" as const,
    },
    controlUi: {
      enabled: true,
      ...(controlUiRoot ? { root: controlUiRoot } : {}),
