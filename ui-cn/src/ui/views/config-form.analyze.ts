...
// Cache for schema analysis — schema object rarely changes, avoid re-analyzing on every render
let _cachedSchemaInput: unknown = null;
let _cachedSchemaResult: ConfigSchemaAnalysis | null = null;

export function analyzeConfigSchema(raw: unknown): ConfigSchemaAnalysis {
  if (!raw || typeof raw !== "object") {
    return { schema: null, unsupportedPaths: ["<root>"] };
  }
  // Return cached result if the same schema object reference is passed
  if (raw === _cachedSchemaInput && _cachedSchemaResult) {
    return _cachedSchemaResult;
  }
  const result = normalizeSchemaNode(raw as JsonSchema, []);
  _cachedSchemaInput = raw;
  _cachedSchemaResult = result;
  return result;
}
...
// -- Secret input schema helpers (aligned with upstream) ----------------------
...
function normalizeUnion(
  ...
  const renderableUnionTypes = new Set(["string", "number", "integer", "boolean", "object", "array"]);
  if (
    remaining.length > 0 &&
    literals.length === 0 &&
