...
export function renderAgentTools(params: {
  ...
  const profileSource = agentTools.profile
    ? t("agents.agentOverride")
    : globalTools.profile
      ? t("agents.globalDefault")
      : t("agents.default");
      ...
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">
            ...
            <span class="mono">${enabledCount}/${toolIds.length}</span> ${t("agents.toolEnabled")}
          </div>
        </div>
          ...
          params.configDirty
            ? html`
                <div class="agent-kv">
                  ...
                  <div class="mono">${t("agents.statusUnsaved")}</div>
                </div>
              `
            : nothing
        }
  ...
  return html`
    <section class="card">
      ...
      <div class="filters" style="margin-bottom: 14px;">
        ...
        <div class="muted">${filtered.length} ${t("agents.shown")}</div>
      </div>
      ...
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agents.customAllowlist")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.allSkillsEnabled")}
              </div>
            `
      }
      ${
      ...
      ${
        filtered.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.noSkills")}</div>
            `
          : html`
...
function renderAgentSkillRow(
  ...
  return html`
    <div class="list-item agent-skill-row">
      <div class="list-main">
        <div class="list-title">${skill.nameZh || skill.name}</div>
        <div class="list-sub">${skill.descriptionZh || skill.description}</div>
        ${renderSkillStatusChips({ skill })}
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skillMissing")} ${missing.join(", ")}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skillReason")} ${reasons.join(", ")}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
