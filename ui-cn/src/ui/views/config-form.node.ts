...
export function renderNode(params: {
  ...
  const help = resolveHelp(path, hint, schema);
  const key = pathKey(path);

  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${t("configForm.unsupportedSchema")}</div>
    </div>`;
  }
  ...
  const placeholder =
    ...
    (isSensitive
      ...
      : schema.default !== undefined
        ? `${t("configForm.defaultPrefix")}${schema.default}`
        : t("configForm.optionalPlaceholder"));
...
function renderObject(params: {
  ...
  // Sort by hint order — pre-lookup orders to avoid O(n²) hintForPath calls in comparator
  const orderMap = new Map<string, number>();
  for (const [propKey] of entries) {
    orderMap.set(propKey, hintForPath([...path, propKey], hints)?.order ?? 0);
  }
  ...
  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">${arr.length} ${t("configForm.items")}</span>
        <button
          type="button"
