...
function buildPeakErrorHours(sessions: UsageSessionEntry[], timeZone: "local" | "utc") {
  ...
  return hourMsgs
    ...
    .map((entry) => ({
      ...
      sub: `${Math.round(entry.errors)} ${t("usage.overview.errorsCount")} · ${Math.round(entry.msgs)} ${t("usage.overview.msgs")}`,
    }));
}
...
function buildUsageMosaicStats(
  ...
  const weekdayLabels = getWeekdays().map((label, index) => ({
    label,
      ...
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            ...
            <div class="usage-mosaic-sub">${t("usage.mosaic.noTimestamps")}</div>
          </div>
          <div class="usage-mosaic-total">${formatTokens(0)} ${t("usage.page.tokens")}</div>
        </div>
        <div class="muted" style="padding: 12px; text-align: center;">${t("usage.mosaic.noDataYet")}</div>
      </div>
    `;
  }
      ...
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            ${t("usage.mosaic.estimatedFrom", { tz: timeZone === "utc" ? "UTC" : t("usage.page.local") })}
          </div>
        </div>
        <div class="usage-mosaic-total">${formatTokens(stats.totalTokens)} ${t("usage.page.tokens")}</div>
      </div>
      <div class="usage-mosaic-grid">
        ...
        <div class="usage-mosaic-section">
          <div class="usage-mosaic-section-title">
            <span>${t("usage.mosaic.hours")}</span>
            <span class="usage-mosaic-sub">${t("usage.mosaic.hoursRange")}</span>
          </div>
          <div class="usage-hour-grid">
            ${stats.hourTotals.map((value, hour) => {
              ...
              const title = `${hour}:00 · ${formatTokens(value)} ${t("usage.page.tokens")}`;
          ...
          <div class="usage-hour-labels">
            ...
            <span>${t("usage.mosaic.4am")}</span>
            <span>${t("usage.mosaic.8am")}</span>
            <span>${t("usage.mosaic.noon")}</span>
            <span>${t("usage.mosaic.4pm")}</span>
            <span>${t("usage.mosaic.8pm")}</span>
          </div>
          <div class="usage-hour-legend">
            <span></span>
            ${t("usage.mosaic.density")}
          </div>
        </div>
      </div>
    </div>
