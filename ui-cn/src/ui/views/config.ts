...
import { hintForPath, humanize, resolveLabel, schemaType, type JsonSchema } from "./config-form.shared";
...
function getSections(): Array<{ key: string; label: string }> {
  return [
    ...
    { key: "cli", label: t("config.cliLabel") },
    { key: "memory", label: t("config.memoryLabel") },
    { key: "acp", label: t("config.acpLabel") },
    { key: "secrets", label: t("config.secretsLabel") },
  ];
}
...
function resolveSubsections(params: {
  ...
  const entries = Object.entries(schema.properties).map(([subKey, node]) => {
    ...
    // 使用 resolveLabel：i18n 优先 → hint.label → schema.title → humanize
    let label = resolveLabel([key, subKey], hint, node, subKey);
    // 对于 channels 分区，如果标签仍是 humanize 产出，优先使用 channel i18n
    if (key === "channels" && CHANNEL_I18N_KEYS[subKey] && label === humanize(subKey)) {
      const channelLabel = tMaybe(CHANNEL_I18N_KEYS[subKey]);
      if (channelLabel !== CHANNEL_I18N_KEYS[subKey]) label = channelLabel;
    }
...
export function renderConfig(props: ConfigProps) {
  // cnPlugin 已迁移到 cn-adapter-state.json，openclaw.json 里不再有 cnPlugin，
  // 上游 schema 校验正常通过，无需过滤。
  ...
  // Sections that already have dedicated tabs in ui-cn navigation — hide from config page
  const SECTIONS_WITH_DEDICATED_TABS = new Set([
    "agents",    // → 智能体 tab
    "models",    // → 模型设置 tab
    "channels",  // → IM渠道 tab
    "skills",    // → 技能 tab
    "cron",      // → 定时任务 tab
    "wizard",    // → 设置向导（model-config tab）
    "ui",        // → 外观随 appearance 处理
  ]);

  const availableSections = sections.filter(
    (s) => s.key in schemaProps && !SECTIONS_WITH_DEDICATED_TABS.has(s.key),
  );
  ...
  const extraSections = Object.keys(schemaProps)
    .filter((k) => !knownKeys.has(k) && !SECTIONS_WITH_DEDICATED_TABS.has(k))
    .map((k) => ({
      key: k,
  ...
  // Quick dirty check — use formDirty flag or reference equality to avoid full diff on every render
  const hasRawChanges = props.formMode === "raw" && props.raw !== props.originalRaw;
  const hasFormChanges =
    props.formMode === "form" && props.formValue !== props.originalValue;
  const hasChanges = hasFormChanges || hasRawChanges;
  // Defer full diff computation — only compute when the diff panel is actually rendered
  // (the diff panel uses <details> which is collapsed by default)
  const lazyDiff = () =>
    props.formMode === "form" ? computeDiff(props.originalValue, props.formValue) : [];
      ...
      <main class="config-main">
        ...
        <div class="config-actions">
          <div class="config-actions__left">
            ${
              hasChanges
                ? html`
              <span class="config-changes-badge">${t("config.unsavedChanges")}</span>
            `
                : html`
        ...
        ${
          hasChanges && props.formMode === "form"
            ? html`
          <details class="config-diff" @toggle=${(e: Event) => {
            const details = e.target as HTMLDetailsElement;
            if (details.open) {
              // Compute diff only when panel is opened
              const diffContent = details.querySelector(".config-diff__content");
              if (diffContent && !diffContent.hasAttribute("data-loaded")) {
                diffContent.setAttribute("data-loaded", "1");
                const diff = lazyDiff();
                diffContent.innerHTML = diff.map(
                  (change) => `
                  <div class="config-diff__item">
                    <div class="config-diff__path">${escapeHtml(change.path)}</div>
                    <div class="config-diff__values">
                      <span class="config-diff__from">${escapeHtml(truncateValue(change.from))}</span>
                      <span class="config-diff__arrow">→</span>
                      <span class="config-diff__to">${escapeHtml(truncateValue(change.to))}</span>
                    </div>
                  </div>`
                ).join("");
              }
            ...
            <summary class="config-diff__summary">
              <span>${t("config.viewPendingChanges", { count: 0 })}</span>
        ...
        <!-- Validation issues banner (shown in both modes when config is invalid) -->
        ${
          props.valid === false && issues.length > 0
            ? html`
              <div class="config-invalid-banner">
                <div class="config-invalid-banner__header">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="config-invalid-banner__icon">
                    <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path>
                    <line x1="12" y1="9" x2="12" y2="13"></line>
                    <line x1="12" y1="17" x2="12.01" y2="17"></line>
                  </svg>
                  <div>
                    <div class="config-invalid-banner__title">${t("config.invalidBannerTitle")}</div>
                    <div class="config-invalid-banner__desc">${
                      props.formMode === "form"
                        ? t("config.invalidBannerDesc")
                        : t("config.invalidBannerRawHint")
                    }</div>
                  </div>
                </div>
                ${
                  props.onRevealConfigFile
                    ? html`<button class="btn btn--sm" @click=${props.onRevealConfigFile}>
                        ${t("config.openConfigFile")}
                      </button>`
                    : nothing
                }
                <details class="config-invalid-banner__details">
                  <summary>${t("config.issueCount", { count: String(issues.length) })}</summary>
                  <pre class="config-invalid-banner__code">${
                    issues.map((issue: unknown) => {
                      const obj = issue as { path?: string; message?: string };
                      return obj.path ? `[${obj.path}] ${obj.message ?? ""}` : JSON.stringify(issue);
                    }).join("\n")
                  }</pre>
                </details>
              </div>
