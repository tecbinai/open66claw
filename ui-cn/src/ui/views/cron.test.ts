...
describe("cron view", () => {
  ...
  it("loads run history when clicking a job card", () => {
    const container = document.createElement("div");
    ...
    // Click on the glass card in the main area
    const card = container.querySelector(".cron-glass-card") as HTMLElement | null;
  ...
  it("marks the selected job and keeps Runs button to a single call", () => {
    ...
    const selected = container.querySelector(".cron-glass-card--selected");
