...
function renderFilterChips(
  ...
  const sessionsLabel = selectedSession
    ...
    : selectedSessions.length === 1
      ...
      : t("usage.chip.sessionsCount", { count: String(selectedSessions.length) });
  ...
  const daysLabel =
    selectedDays.length === 1
      ? selectedDays[0]
      : t("usage.chip.daysCount", { count: String(selectedDays.length) });
  const hoursLabel =
    selectedHours.length === 1
      ? `${selectedHours[0]}:00`
      : t("usage.chip.hoursCount", { count: String(selectedHours.length) });
    ...
    <div class="active-filters">
        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usage.chip.days", { label: daysLabel })}</span>
              <button class="filter-chip-remove" @click=${onClearDays} title=${t("usage.page.removeFilter")}>×</button>
            </div>
          `
          : nothing
      }
      ${
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${t("usage.chip.hours", { label: hoursLabel })}</span>
              <button class="filter-chip-remove" @click=${onClearHours} title=${t("usage.page.removeFilter")}>×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              <span class="filter-chip-label">${t("usage.chip.session", { label: sessionsLabel })}</span>
              <button class="filter-chip-remove" @click=${onClearSessions} title=${t("usage.page.removeFilter")}>×</button>
            </div>
          `
          : nothing
      }
  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">${t("usage.daily.title", { mode: "Token" })}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.daily.noData")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">${t("usage.daily.title", { mode: isTokenMode ? "Token" : t("usage.page.costToggle") })}</div>
      </div>
          ...
          ${daily.map((d, idx) => {
            ...
            const breakdownLines =
              dailyChartMode === "by-type"
                ? isTokenMode
                  ? [
                      `${t("usage.daily.output")} ${formatTokens(d.output)}`,
                      `${t("usage.daily.input")} ${formatTokens(d.input)}`,
                      `${t("usage.daily.cacheWrite")} ${formatTokens(d.cacheWrite)}`,
                      `${t("usage.daily.cacheRead")} ${formatTokens(d.cacheRead)}`,
                    ]
                  : [
                      `${t("usage.daily.output")} ${formatCost(d.outputCost ?? 0)}`,
                      `${t("usage.daily.input")} ${formatCost(d.inputCost ?? 0)}`,
                      `${t("usage.daily.cacheWrite")} ${formatCost(d.cacheWriteCost ?? 0)}`,
                      `${t("usage.daily.cacheRead")} ${formatCost(d.cacheReadCost ?? 0)}`,
                    ]
                : [];
            ...
            return html`
              ...
              >
                ...
                <div class="daily-bar-tooltip">
                  ...
                  ${formatTokens(d.totalTokens)} ${t("usage.page.tokens")}<br />
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? t("usage.breakdown.tokensByType") : t("usage.breakdown.costByType")}</div>
...
function renderUsageInsights(
  ...
  const avgDurationLabel =
    stats.durationCount > 0
      ...
      : "—";
  const cacheHint = t("usage.overview.cacheHitRateHint");
  const errorHint = t("usage.overview.errorRateHint");
  const throughputHint = t("usage.overview.throughputHint");
  const tokensHint = t("usage.overview.avgTokensMsgHint");
  const costHint = showCostHint
    ? t("usage.overview.avgCostMsgMissing")
    : t("usage.overview.avgCostMsgDefault");
  ...
  const errorDays = aggregates.daily
    ...
    .map((day) => {
      ...
      return {
        ...
        sub: `${day.errors} ${t("usage.overview.errorsCount")} · ${day.messages} ${t("usage.overview.msgs")} · ${formatTokens(day.tokens)}`,
        rate,
      };
    })
  ...
  const topModels = aggregates.byModel.slice(0, 5).map((entry) => ({
    ...
    sub: `${formatTokens(entry.totals.totalTokens)} · ${entry.count} ${t("usage.overview.msgs")}`,
  }));
      ...
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${t("usage.overview.userAssistant", { user: String(aggregates.messages.user), assistant: String(aggregates.messages.assistant) })}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${t("usage.overview.toolsUsed", { count: String(aggregates.tools.uniqueTools) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${t("usage.overview.toolResults", { count: String(aggregates.messages.toolResults) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.avgTokensMsg")}
            <span class="usage-summary-hint" title=${tokensHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${t("usage.overview.acrossMessages", { count: String(aggregates.messages.total || 0) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.avgCostMsg")}
            <span class="usage-summary-hint" title=${costHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatCost(avgCost, 4)}</div>
          <div class="usage-summary-sub">${formatCost(totals.totalCost)} ${t("usage.overview.total")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.sessionsCard")}
            <span class="usage-summary-hint" title=${t("usage.overview.sessionsHint")}>?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">${t("usage.overview.ofTotal", { total: String(totalSessions) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${t("usage.overview.errorsAvgSession", { errors: String(aggregates.messages.errors), duration: avgDurationLabel })}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">
            ${t("usage.overview.cachedPrompt", { cached: formatTokens(totals.cacheRead), prompt: formatTokens(cacheBase) })}
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ${renderInsightList(t("usage.overview.topModels"), topModels, t("usage.overview.noModelData"))}
        ${renderInsightList(t("usage.overview.topProviders"), topProviders, t("usage.overview.noProviderData"))}
        ${renderInsightList(t("usage.overview.topTools"), topTools, t("usage.overview.noToolCalls"))}
        ${renderInsightList(t("usage.overview.topAgents"), topAgents, t("usage.overview.noAgentData"))}
        ${renderInsightList(t("usage.overview.topChannels"), topChannels, t("usage.overview.noChannelData"))}
        ${renderPeakErrorList(t("usage.overview.peakErrorDays"), errorDays, t("usage.overview.noErrorData"))}
        ${renderPeakErrorList(t("usage.overview.peakErrorHours"), errorHours, t("usage.overview.noErrorData"))}
      </div>
    </section>
  `;
}
    ...
    <div class="card sessions-card">
      <div class="sessions-card-header">
        ...
        <div class="sessions-card-count">
          ${totalSessions !== sessions.length ? t("usage.sessions.shownTotal", { shown: String(sessions.length), total: String(totalSessions) }) : t("usage.sessions.shown", { shown: String(sessions.length) })}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} ${t("usage.sessions.avg")}</span>
          <span>${totalErrors} ${t("usage.sessions.errors")}</span>
        </div>
        ...
        <label class="sessions-sort">
          ...
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>${t("usage.sessions.sortCost")}</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>${t("usage.sessions.sortErrors")}</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>${t("usage.sessions.sortMessages")}</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>${t("usage.sessions.sortRecent")}</option>
            <option value="tokens" ?selected=${sessionSort === "tokens"}>${t("usage.sessions.sortTokens")}</option>
          </select>
        </label>
        <button
          class="btn btn-sm sessions-action-btn icon"
          @click=${() => onSessionSortDirChange(sessionSortDir === "desc" ? "asc" : "desc")}
          title=${sessionSortDir === "desc" ? t("usage.sessions.descending") : t("usage.sessions.ascending")}
        >
          ${sessionSortDir === "desc" ? "↓" : "↑"}
        </button>
        ${
      ...
      ${
        sessionsTab === "recent"
          ? recentEntries.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usage.sessions.noRecent")}</div>
              `
            : html`
          ...
          : sessions.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usage.sessions.noInRange")}</div>
              `
            : html`
	                <div class="session-bars">
	                  ...
	                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">${t("usage.sessions.more", { count: String(sessions.length - 50) })}</div>` : nothing}
	                </div>
	              `
      }
      ${
        selectedCount > 1
          ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${t("usage.sessions.selected", { count: String(selectedCount) })}</div>
