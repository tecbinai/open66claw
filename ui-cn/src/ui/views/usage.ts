...
export function renderUsage(props: UsageProps) {
        ...
        <div class="row" style="justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 12px;">
          <div style="flex: 1; min-width: 250px;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2px;">
              <div class="card-title" style="margin: 0;">${t("usage.page.title")}</div>
              <span style="
                display: inline-flex;
                align-items: center;
                gap: 6px;
          ...
          <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;">
            <div style="display: flex; gap: 8px; align-items: center;">
              ...
              <span style="color: var(--text-muted);">${t("usage.page.to")}</span>
  ...
  const hasMissingCost =
    ...
    (displayTotals
      ...
      : false);
  const datePresets = [
    { label: t("usage.page.today"), days: 1 },
    { label: "7d", days: 7 },
    { label: "30d", days: 30 },
  ];
        ...
        <summary>
          ...
          ${
            selectedCount > 0
              ...
              : html`
                  <span class="usage-filter-badge">${t("usage.page.all")}</span>
                `
          }
        </summary>
    ...
    <section class="card usage-header ${props.headerPinned ? "pinned" : ""}">
      <div class="usage-header-row">
        <div class="usage-header-title">
          <div class="card-title" style="margin: 0;">${t("usage.page.filters")}</div>
          ${
            props.loading
              ? html`
                  <span class="usage-refresh-indicator">${t("usage.page.loading")}</span>
                `
              : nothing
          }
          ${
            isEmpty
              ? html`
                  <span class="usage-query-hint">${t("usage.page.loadingHint")}</span>
                `
              : nothing
          }
        </div>
        <div class="usage-header-metrics">
            ...
            displayTotals
              ? html`
                <span class="usage-metric-badge">
                  <strong>${formatTokens(displayTotals.totalTokens)}</strong> ${t("usage.page.tokens")}
                </span>
                <span class="usage-metric-badge">
                  <strong>${formatCost(displayTotals.totalCost)}</strong> ${t("usage.page.cost")}
                </span>
                <span class="usage-metric-badge">
                  <strong>${displaySessionCount}</strong>
                  ${t("usage.page.sessions")}
                </span>
              `
              : nothing
          }
          <button
            ...
            title=${props.headerPinned ? t("usage.page.unpinFilters") : t("usage.page.pinFilters")}
            @click=${props.onToggleHeaderPinned}
          >
            ${props.headerPinned ? t("usage.page.pinned") : t("usage.page.pin")}
          </button>
          <details
          ...
          >
            <summary class="usage-export-button">${t("usage.page.export")} ▾</summary>
            <div class="usage-export-popover">
              <div class="usage-export-list">
                ...
                >
                  ${t("usage.page.exportSessionsCsv")}
                </button>
                <button
                  class="usage-export-item"
                  @click=${() =>
      ...
      <div class="usage-header-row">
        <div class="usage-controls">
          ...
          <div class="chart-toggle">
            ...
            >
              ${t("usage.page.tokensToggle")}
            </button>
            <button
              class="toggle-btn ${!isTokenMode ? "active" : ""}"
              @click=${() => props.onChartModeChange("cost")}
            >
              ${t("usage.page.costToggle")}
            </button>
          </div>
          <button
      ...
      <div style="margin-top: 12px;">
          ...
          <input
            ...
            placeholder=${t("usage.page.filterPlaceholder")}
          ...
          <div class="usage-query-actions">
            ...
            >
              ${t("usage.page.filterClientSide")}
            </button>
            ${
              hasDraftQuery || hasQuery
                ? html`<button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${props.onClearQuery}>${t("usage.page.clear")}</button>`
                : nothing
            }
            <span class="usage-query-hint">
              ${
                hasQuery
                  ? t("usage.page.matchCount", {
                      matched: String(filteredSessions.length),
                      total: String(totalSessions),
                    })
                  : t("usage.page.sessionCount", { total: String(totalSessions) })
              }
            </span>
          </div>
        </div>
        <div class="usage-filter-row">
          ${renderFilterSelect("agent", t("usage.page.agent"), agentOptions)}
          ${renderFilterSelect("channel", t("usage.page.channel"), channelOptions)}
          ${renderFilterSelect("provider", t("usage.page.provider"), providerOptions)}
          ${renderFilterSelect("model", t("usage.page.model"), modelOptions)}
          ${renderFilterSelect("tool", t("usage.page.tool"), toolOptions)}
          <span class="usage-query-hint">
            ${t("usage.page.filterTip")}
          </span>
        </div>
        ${
          queryTerms.length > 0
                  ...
                  ${queryTerms.map((term) => {
                    ...
                    return html`
                      <span class="usage-query-chip">
                        ${label}
                        <button
                          title=${t("usage.page.removeFilter")}
                          @click=${() =>
        ...
        props.sessionsLimitReached
          ? html`
              <div class="callout warning" style="margin-top: 12px">
                ${t("usage.page.sessionsLimitWarning")}
              </div>
            `
          : nothing
      }
