...
import { formatDurationCompact } from "../../shared/format-duration.js";
...
function renderSessionSummary(session: UsageSessionEntry) {
  ...
  if (!usage) {
    return html`
      <div class="muted">${t("usage.detail.noUsageData")}</div>
    `;
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.detail.messages")}</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">${t("usage.detail.userAssistant", { user: String(usage.messageCounts?.user ?? 0), assistant: String(usage.messageCounts?.assistant ?? 0) })}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.detail.toolCalls")}</div>
        <div class="session-summary-value">${usage.toolUsage?.totalCalls ?? 0}</div>
        <div class="session-summary-meta">${usage.toolUsage?.uniqueTools ?? 0} ${t("usage.detail.tools")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.detail.errors")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} ${t("usage.detail.toolResults")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.detail.duration")}</div>
    ...
    <div class="usage-insights-grid" style="margin-top: 12px;">
      ${renderInsightList(t("usage.detail.topTools"), toolItems, t("usage.detail.noToolCalls"))}
      ${renderInsightList(t("usage.detail.modelMix"), modelItems, t("usage.detail.noModelData"))}
    </div>
  `;
}
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> ${t("usage.page.tokens")}</span>
            <span><strong>${formatCost(usage.totalCost)}</strong></span>
          `
              : nothing
          }
        </div>
        <button class="session-close-btn" @click=${onClose} title=${t("usage.detail.closeTitle")}>×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  if (loading) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.timeseries.loading")}</div>
      </div>
    `;
  }
  if (!timeSeries || timeSeries.points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.timeseries.noData")}</div>
      </div>
    `;
  }
  ...
  if (points.length < 2) {
    return html`
      <div class="session-timeseries-compact">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.timeseries.noDataInRange")}</div>
      </div>
    `;
  }
  let cumTokens = 0,
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 13px;">${t("usage.timeseries.title")}</div>
        <div class="timeseries-controls">
          <div class="chart-toggle small">
            ...
            >
              ${t("usage.timeseries.perTurn")}
            </button>
            <button
              class="toggle-btn ${isCumulative ? "active" : ""}"
              @click=${() => onModeChange("cumulative")}
            >
              ${t("usage.timeseries.cumulative")}
            </button>
          </div>
          ${
            !isCumulative
      ...
      <svg viewBox="0 0 ${width} ${height + 15}" class="timeseries-svg" style="width: 100%; height: auto;">
        ...
        ${points.map((p, i) => {
          ...
          const tooltipLines = [
            ...
            `${formatTokens(val)} ${t("usage.page.tokens")}`,
          ];
          if (breakdownByType) {
            tooltipLines.push(`${t("usage.timeseries.output")} ${formatTokens(p.output)}`);
            tooltipLines.push(`${t("usage.timeseries.input")} ${formatTokens(p.input)}`);
            tooltipLines.push(`${t("usage.timeseries.cacheWrite")} ${formatTokens(p.cacheWrite)}`);
            tooltipLines.push(`${t("usage.timeseries.cacheRead")} ${formatTokens(p.cacheRead)}`);
          }
        ...
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px;">${t("usage.timeseries.tokensByType")}</div>
                ...
                <div class="cost-breakdown-legend">
                  <div class="legend-item" title=${t("usage.timeseries.outputHint")}>
                    <span class="legend-dot output"></span>${t("usage.timeseries.output")} ${formatTokens(sumOutput)}
                  </div>
                  <div class="legend-item" title=${t("usage.timeseries.inputHint")}>
                    <span class="legend-dot input"></span>${t("usage.timeseries.input")} ${formatTokens(sumInput)}
                  </div>
                  <div class="legend-item" title=${t("usage.timeseries.cacheWriteHint")}>
                    <span class="legend-dot cache-write"></span>${t("usage.timeseries.cacheWrite")} ${formatTokens(sumCacheWrite)}
                  </div>
                  <div class="legend-item" title=${t("usage.timeseries.cacheReadHint")}>
                    <span class="legend-dot cache-read"></span>${t("usage.timeseries.cacheRead")} ${formatTokens(sumCacheRead)}
                  </div>
                </div>
                <div class="cost-breakdown-total">${t("usage.breakdown.total")} ${formatTokens(totalTypeTokens)}</div>
              </div>
            `
          : nothing
      }
...
function renderContextPanel(
  ...
  if (!contextWeight) {
    return html`
      <div class="context-details-panel">
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.context.noData")}</div>
      </div>
    `;
  }
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.context.ofInput", {
        pct: String(Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0)),
      });
    }
  }
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 13px;">${t("usage.context.title")}</div>
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usage.context.collapse") : t("usage.context.expandAll")}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">${contextPct || t("usage.context.baseContext")}</p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.context.sys")}: ~${formatTokens(systemTokens)}"></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.context.skills")}: ~${formatTokens(skillsTokens)}"></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.context.tools")}: ~${formatTokens(toolsTokens)}"></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.context.files")}: ~${formatTokens(filesTokens)}"></div>
      </div>
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>${t("usage.context.sys")} ~${formatTokens(systemTokens)}</span>
        <span class="legend-item"><span class="legend-dot skills"></span>${t("usage.context.skills")} ~${formatTokens(skillsTokens)}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usage.context.tools")} ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.context.files")} ~${formatTokens(filesTokens)}</span>
      </div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.skillsCount", { count: String(skillsList.length) })}</div>
                    ...
                    ${
                      more > 0
                        ? html`<div class="context-breakdown-more">${t("usage.context.moreCount", { count: String(more) })}</div>`
                        : nothing
                    }
                  </div>
                `;
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.toolsCount", { count: String(toolsList.length) })}</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.filesCount", { count: String(filesList.length) })}</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.logs.title")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.timeseries.loading")}</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.logs.title")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.logs.noMessages")}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.logs.title")} <span style="font-weight: normal; color: var(--text-muted);">${t("usage.logs.messagesCount", { count: displayedCount })}</span></span>
        <button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${onToggleExpandedAll}>
          ${expandedAll ? t("usage.logs.collapseAll") : t("usage.logs.expandAll")}
        </button>
      </div>
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usage.logs.user")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("usage.logs.assistant")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("usage.logs.toolOption")}</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>${t("usage.logs.toolResultOption")}</option>
        </select>
        <select
          multiple
          size="4"
        ...
        <label class="usage-filters-inline" style="gap: 6px;">
          ...
          />
          ${t("usage.logs.hasTools")}
        </label>
        <input
          type="text"
          placeholder=${t("usage.logs.searchPlaceholder")}
          .value=${filters.query}
      ...
      <div class="session-logs-list">
        ${filteredEntries.map((entry) => {
          ...
          const roleLabel =
            log.role === "user"
              ? t("usage.logs.roleYou")
              : log.role === "assistant"
                ? t("usage.logs.roleAssistant")
                : t("usage.logs.roleTool");
          return html`
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usage.logs.noMatch")}</div>
              `
            : nothing
        }
      </div>
