...
} from "../shared/client-info.js";
import { buildDeviceAuthPayload } from "../shared/gateway-device-auth.js";
...
export class GatewayBrowserClient {
  ...
  request<T = unknown>(method: string, params?: unknown): Promise<T> {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.warn("[gw-ws] request REJECTED: not connected, method=", method);
      return Promise.reject(new Error("gateway not connected"));
    }
    const id = generateUUID();
    const frame = { type: "req", id, method, params };
    // Client-side timeout: if the gateway doesn't respond within 30s,
    // reject the promise so callers don't hang forever (e.g. channelsLoading stuck).
    const timeoutMs = (params as { timeoutMs?: number } | undefined)?.timeoutMs;
    const effectiveTimeout = Math.max(timeoutMs ?? 30_000, 10_000) + 5_000; // params.timeoutMs + 5s grace
    console.log("[gw-ws] request SEND method=", method, "id=", id, "timeout=", effectiveTimeout);
    const p = new Promise<T>((resolve, reject) => {
      const timer = window.setTimeout(() => {
        if (this.pending.delete(id)) {
          console.error("[gw-ws] request TIMEOUT method=", method, "id=", id);
      ...
      this.pending.set(id, {
        resolve: (v) => {
          ...
          console.log("[gw-ws] request OK method=", method, "id=", id);
          resolve(v as T);
        },
        reject: (err) => {
          window.clearTimeout(timer);
          console.error("[gw-ws] request FAIL method=", method, "id=", id, err);
          reject(err);
        },
      });
    });
