...
type ToolStreamHost = {
  ...
  /** [CN-PATCH:media-tool-heartbeat] Auto-set when a media generation tool is running. */
  chatMediaToolActive: { tool: string; args?: Record<string, unknown> } | null;
};

/** Tool names that represent long-running media generation tasks. */
const MEDIA_TOOL_NAMES = new Set(["video_gen", "image_gen"]);
...
export function handleCompactionEvent(host: CompactionHost, payload: AgentEventPayload) {
  ...
  const contextPercent = typeof data.contextPercent === "number" ? data.contextPercent : undefined;
...
export function handleAgentEvent(host: ToolStreamHost, payload?: AgentEventPayload) {
  ...
  // [CN-PATCH:media-tool-heartbeat] Auto-detect media generation tools so the UI
  // shows a media-pending shimmer card instead of the 90s timeout reading-indicator.
  if (MEDIA_TOOL_NAMES.has(name)) {
    if (phase === "start") {
      host.chatMediaToolActive = {
        tool: name,
