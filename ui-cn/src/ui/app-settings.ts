...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "skills") {
    const _tSkills = performance.now();
    await loadSkills(host as unknown as OpenClawCNApp);
    console.log(
      `[perf][UI] refreshActiveTab("skills") = ${(performance.now() - _tSkills).toFixed(1)}ms`,
    );
  }
  if (host.tab === "agents") {
  ...
  if (host.tab === "extensions") {
    console.log(`[perf][UI] refreshActiveTab("extensions") START`);
    ...
    if (
      app.mcpMarketplace &&
      app.mcpMarketplace.items.length === 0 &&
      !app.mcpMarketplace.loading
    ) {
      const callbacks = {
