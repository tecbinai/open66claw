...
/**
 * Shallow-clone only the objects along the given path (copy-on-write).
 * Much cheaper than a full deep clone when updating a single value.
 */
export function shallowCloneAtPath(
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (typeof key === "number") {
      ...
      // Clone the element at this index, not the array itself.
      // The parent array was already cloned in the preceding string-key step.
      ...
      if (child != null && typeof child === "object") {
        ...
        current[key] = copy;
        current = copy as Record<string, unknown> | unknown[];
      } else {
        break;
      }
    } else {
