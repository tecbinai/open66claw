...
### CN 版本开发 (ui-cn + cn-adapter)

#### CLI 方式（浏览器访问）

```bash
pnpm install
pnpm cn:gateway                 # 启动 CN gateway，浏览器打开 http://localhost:18789
```

`pnpm cn:gateway` 自动完成 CN 插件配置、ui-cn 构建（检测源码变更自动重新构建）和 gateway 启动。`git pull` 后直接运行即可，无需手动重新构建。

#### 桌面端方式（Tauri 应用）

```bash
pnpm install
pnpm build                      # 构建后端 + CN 前端（自动执行 ui-cn:build）
cd apps/desktop && npx tauri dev  # 开发模式（热重载，图标可正常显示）
cd apps/desktop && npx tauri build  # 打包 ARM64 .app/.dmg
cd apps/desktop && npx tauri build --target x86_64-apple-darwin  # 打包 Intel Mac
```

桌面端启动流程：加载 splash 页面 → sidecar 启动 gateway（端口 19002）→ 健康检查通过后导航到 CN UI。

打包说明：
- `npx tauri build` 自动执行 `stage-dist.sh`，将后端 JS + CN 前端打包进 `.app` Resources
- 打包后的 `.app` 可安装到任意目录独立运行（需系统安装 Node.js）
- `tauri dev` 模式下 sidecar 回退到系统 node + 仓库 `dist/entry.js`
- CLI gateway 端口 **18789**，桌面端端口 **19002**
- `pnpm build` 末尾自动执行 `pnpm ui-cn:build`，确保 CN UI 不被上游覆盖
- **不要单独执行 `pnpm ui:build`**，否则会用上游 UI 覆盖 CN UI
- 详见 CLAUDE.md 中的「CN 版本本地开发」章节

### 66claw 文档 (66claw-docs/)

`66claw-docs/` 目录存放 66claw（CN 版）相关的说明文档，包括：

- [ui-cn-feature-overview.md](66claw-docs/ui-cn-feature-overview.md) — ui-cn 功能全景与 OpenClaw 差异对照
- [custom-features-detail.md](66claw-docs/custom-features-detail.md) — 66claw 自定义功能详解与排查报告（18 个功能模块）
- [custom-files-inventory.md](66claw-docs/custom-files-inventory.md) — 自定义文件清单（非 OpenClaw 原生）
- [architecture-overview.md](66claw-docs/architecture-overview.md) — 架构总览
- [code-ownership-analysis.md](66claw-docs/code-ownership-analysis.md) — 代码归属分析
- [desktop-icon-guide.md](66claw-docs/desktop-icon-guide.md) — 桌面端图标指南
- [desktop-release-checklist.md](66claw-docs/desktop-release-checklist.md) — 桌面端发布检查清单
